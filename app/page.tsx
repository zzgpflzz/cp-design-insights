...
type TabType = 'monthly' | 'roadmap' | 'uiux' | 'tf';
...
export default function Playground() {
  ...
  const fetchTFTasks = useCallback(async () => {
    console.log('🔵 fetchTFTasks started');
    const startTime = performance.now();
    try {
      const q = query(collection(db, 'tfTasks'), orderBy('createdAt', 'desc'));
      const querySnapshot = await getDocs(q);
      console.log(`✅ TFTasks fetched: ${querySnapshot.docs.length} docs in ${(performance.now() - startTime).toFixed(2)}ms`);
                    ...
                    {project.releaseDate && (
                      <div className="mb-3">
                        ...
                        >
                          ...
                          <span className={`text-xs ${isCompleted ? 'font-bold' : ''}`}>
                        ...
                        <div className="space-y-3 flex-1">
                          {axTasks.length === 0 ? (
                            ...
                            axTasks.map((task) => (
                              <div
                                key={task.id}
                                className="p-4 bg-gray-50 rounded-lg border border-gray-200 hover:border-[#00A6FF] hover:shadow-md transition-all"
                              >
                                <div className="flex items-start justify-between mb-2">
                                  <h4 className="text-base font-bold text-[#313131] flex-1">{task.title}</h4>
                                  <span className="text-xs px-2.5 py-1 rounded-[6px] font-medium ml-2"
